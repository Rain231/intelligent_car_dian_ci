...
/********PID中间变量声明************/
extern PID  AngleVelPID; //角速度环PID储存（只是储存的中间量，无需关心）
extern PID  AnglePID ;//角度环PID储存（只是储存的中间量，无需关心）
extern PID SpeedPID;//速度PID储存（只是储存的中间量，无需关心）
extern PID DirectVelPID;//转向内环PID储存（只是储存的中间量，无需关心）
...
extern float DirectVel_Pid[4];	// 转向内环PID 位置	
